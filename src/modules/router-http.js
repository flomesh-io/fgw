...
function makeRouter(listener, routeResources, gateway) {
  ...
  hostFullnames = Object.fromEntries(
    Object.entries(hostFullnames).map(
      ([k, v]) => [k, makeRuleSelector(v, listener, gateway)]
    )
  )

  hostPostfixes = Object.entries(hostPostfixes)
    .sort((a, b) => b[0].length - a[0].length)
    .map(([k, v]) => [k, makeRuleSelector(v, listener, gateway)])
...
function makeRuleSelector(routeResources, listener, gateway) {
  var kind = routeResources[0].kind
  var rules = routeResources.flatMap(resource => resource.spec.rules.map(
    r => [r, makeBackendSelectorForRule(r, kind === 'GRPCRoute', listener, gateway), resource]
  ))
  ...
  switch (kind) {
    case 'HTTPRoute':
      matches = matches.map(
        ([m, backendSelector, resource, rule]) => [makeHTTPMatcher(m, resource, rule), backendSelector, m]
      )
      break
    case 'GRPCRoute':
      matches = matches.map(
        ([m, backendSelector]) => [makeGRPCMatcher(m), backendSelector, m]
      )
      break
...
function makeHTTPMatcher(match, resource, rule) {
  var matchMethod = makeMethodMatcher(match.method)
  var matchPath = makePathMatcher(match.path)
  var matchHeaders = makeObjectMatcher(match.headers)
  var matchParams = makeObjectMatcher(match.queryParams)
  return function (head) {
...
function makeGRPCMatcher(match) {
  var matchMethod = makeGRPCMethodMatcher(match.method)
  var matchHeaders = makeObjectMatcher(match.headers)
  return function (head) {
...
function makeBackendSelectorForRule(rule, isHTTP2, listener, gateway) {
