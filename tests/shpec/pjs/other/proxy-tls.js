((
  ...
  // curl -v -k -H "a: 1" -H "b: 2" "http://www.test.com:8087/repo/xxx/yyy/?abc=1" -H "host: www.test.com"
  ...
  xxxx = console.log(getParameters('?aaaa=b&111=Abc&')),

) => pipy({
  _port: {bpsLimit: 10000000},
  _clientCert: null,
  _clientSni: null,
})
...
.throttleDataRate(
  () => (
    console.log('..........throttle............', _port.bpsLimit),
    new algo.Quota(
      _port.bpsLimit,
      {
        produce: _port.bpsLimit,
        per: 1,
      }
    )
  )
...
.connectTLS({
  ...
  sni: () => 'www.test.com',
  verify: (ok, cert) => (
    console.log('-------------xxxxxxx 8087,', ok, cert),
    ok
  )
}).to(
