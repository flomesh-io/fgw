...
describe "SHPEC consumer variables Test"
  start="$(date +%s%3N)"
  output_1="$(curl -v http://127.0.0.1:8080/ -H 'apikey: 3e671ea34dcac32e7e9e7c67ee8cfc0b' >/dev/null 2>&1)"
  sleep 1
  pipylog="$(cat tmp/pipy.log)"
  end="$(date +%s%3N)"
  runtime=$((end-start))

  it "check consumer name"
    assert grep "$pipylog" '"x-app-id": "kms"'
  end

  it "check path rewrite"
    assert grep "$pipylog" 'path: "/app1/"'
  end
