...
describe "SHPEC HTTP cookie sticky Test"
  start="$(date +%s%3N)"
  output_3="$(curl -v http://127.0.0.1:8080/ -H 'cookie: _srv_id=8153145845723989' 2>/dev/null)"
  output_4="$(curl -v http://127.0.0.1:8080/ -H 'cookie: _srv_id=8153145845723989' 2>/dev/null)"
  pipylog="$(cat tmp/pipy.log | grep refused)"
  end="$(date +%s%3N)"
  runtime=$((end-start))

  it "check www2"
    assert grep "$output_3" '<p>www2</p>'
  end

  it "check www2 again"
    assert grep "$output_4" '<p>www2</p>'
  end
