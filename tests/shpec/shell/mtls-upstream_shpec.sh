...
describe "SHPEC mTLS upstream Test"
  start="$(date +%s%3N)"
  output_1="$(curl -s -i -k --http1.1 --resolve www.test.com:443:127.0.0.1 https://www.test.com/)"
  output_2="$(curl -s -i -k --http1.1 http://127.0.0.1/path/abc)"
  curl -s -i -k --http1.1 http://127.0.0.1/path/abc/tail
  code=$?
  end="$(date +%s%3N)"
  runtime=$((end-start))

  it "client certificate case 1"
    assert grep "$output_1" "www-port-8850"
  end

  it "client certificate case 2"
    assert grep "$output_2" "www-port-8850"
  end

  it "bad client certificate"
