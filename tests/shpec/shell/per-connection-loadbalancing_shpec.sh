...
describe "SHPEC per-connection loadbalancing Test"

  start="$(date +%s%3N)"
  output1="$(curl localhost:8083/ --next localhost:8083/ --next localhost:8083 --next  localhost:8083 2>&1 | sort | uniq -c | grep '4 port808' | wc | awk '{print $1}')"
  output2="$(curl localhost:8084/ --next localhost:8084/ --next localhost:8084 --next  localhost:8084 2>&1 | sort | uniq -c | grep '4 port808' | wc | awk '{print $1}')"
  end="$(date +%s%3N)"
  runtime=$((end-start))

  it "check HTTP1.1"
    assert equal $output1 "1"
  end

  it "check HTTP2"
    assert equal $output2 "1"
  end
