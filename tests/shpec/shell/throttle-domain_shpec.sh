...
describe "SHPEC throttle domain Test"

  start="$(date +%s%3N)"
  curl -s -i http://127.0.0.1/ > tmp/1.log
  curl -s -i http://127.0.0.1/ > tmp/2.log
  curl -s -i http://127.0.0.1/ > tmp/3.log &
  sleep 1
  curl -s -i http://127.0.0.1/ > tmp/4.log &
  sleep 1
  curl -s -i http://127.0.0.1/ > tmp/5.log &
  ...
  it "5: 431 Request"
    assert grep "$output_5" "431 Request"
  end
